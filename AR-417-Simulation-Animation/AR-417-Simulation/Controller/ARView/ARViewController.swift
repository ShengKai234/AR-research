...
//        })
    ...
    @IBAction func btn_animation(_ sender: Any) {
        if (!isPlay){
            ...
            // 從時間挑選器取得初始時間
            timeAnimation = pickerdate.date
            // 隱藏時間挑選器
            pickerdate.isHidden = true
            // 顯示時間顯示label
            label_time.isHidden = false
            isPlay = true
            label_time.text = formatter.string(from: timeAnimation!)
            btn_animation.setTitle("Pause", for: .normal)
            ...
            // 隱藏時間顯示label
            label_time.isHidden = true
            label_time.text = ""
            isPlay = false
            label_time.text = "Stop"
            btn_animation.setTitle("Animation", for: .normal)
            timer.invalidate()
        }
    }
    @objc func timerAction() {
        ...
        for workSchedule in self.workSchedules{
            ...
            let objectDate = formatter.date(from: workSchedule.startDate)
    ...
    func getNode(withImageName name: String, type: String) -> SCNNode {
        ...
        case "column":
            ...
            case "ElectromechanicalModel":
                ...
                currentElectromechanicalNode?.isHidden = false
                break
            case "Furniture":
                node = FurnitureNode
                currentFurnitureNode = FurnitureNode
                currentFurnitureNode?.isHidden = false
                break
            case "Struct":
                node = StructNode
                currentStructNode = StructNode
                currentStructNode?.isHidden = false
                break
            default:
                break
            }
